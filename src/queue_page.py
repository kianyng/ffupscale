...
class QueueJobRow(QFrame):
    ...
    def update_job(
        ...
        fps_text = (
            "Original FPS"
            if job.fps is None
            else f"{job.fps:g} FPS"
        )

        self.details_label.setText(
            (
                f"{job.resolution_text} · "
                f"{fps_text} · "
                f"{job.encoder}"
            )
        )

        status_colour = STATUS_COLOURS[
            job.status
        ]
    ...
    def set_jobs(
        ...
        for job in jobs:
            ...
            row.action_requested.connect(
                self.job_action_requested.emit
            )
            row.remove_requested.connect(
                self.remove_job_requested.emit
            )
